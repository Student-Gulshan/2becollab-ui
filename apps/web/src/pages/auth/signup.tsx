...
export function SignupPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#F8F7FC] py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center selection:bg-purple-100 selection:text-[#5125D8]">
      ...
      <div className="max-w-[1140px] w-full bg-white rounded-[32px] overflow-hidden shadow-2xl shadow-[#5125D8]/8 border border-slate-200/80 grid grid-cols-1 lg:grid-cols-12 min-h-[640px]">

        {/* ════════════════════════════════════════════════════════════
            LEFT PANEL: Deep Purple Brand Gradient Canvas (Classy & Clean)
            Logo at top, Join Us badge, Headline, 3 Step Cards at bottom
            ════════════════════════════════════════════════════════════ */}
        <div
        ...
        >
          ...
          <div className="relative z-10 my-10 lg:my-0">
            ...
            <p className="text-xs sm:text-sm text-purple-100/90 font-normal leading-relaxed max-w-sm">
              Follow these simple steps to set up your account and start collaborating with verified partners.
            </p>
          </div>

          {/* Bottom 3 Step Cards (Matching reference screenshot) */}
          <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-white/10">
            ...
            <div className="bg-white rounded-2xl p-4 text-[#17213B] shadow-xl">
              ...
              <p className="text-[10px] text-[#687087] mt-1 leading-snug">
                Fast setup with email or Google
              </p>
            </div>
        ...
        {/* ════════════════════════════════════════════════════════════
            ...
            single login link below button, and single legal disclaimer
            ════════════════════════════════════════════════════════════ */}
        <div className="lg:col-span-6 p-8 sm:p-10 lg:p-12 flex flex-col justify-center bg-white">
          <div className="max-w-md w-full mx-auto">
            ...
            <p className="text-xs sm:text-sm text-[#687087] mb-6 font-normal">
              Create your 2BeCollab account in seconds.
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[#17213B] mb-1.5">
                  {isCreator ? 'Full Name' : 'Company / Brand Name'}
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    type="text"
                    required
                    placeholder={isCreator ? 'Enter your full name' : 'Enter company or brand name'}
                    value={fullName}
              ...
              <div>
                ...
                {confirmPassword.length > 0 && password !== confirmPassword && (
                  <p className="text-[11px] text-red-500 mt-1 pl-1 font-medium">Passwords do not match</p>
                )}
              </div>

              <p className="text-[11px] text-[#687087] leading-tight font-normal">
              ...
              <div className="pt-2">
                ...
                >
                  <span>{registerMutation.isPending ? 'Creating Account...' : 'Continue'}</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>

              {/* Single Clean Login Link (Directly below button, exactly like reference) */}
              ...
              {/* Single Legal Disclaimer (At the very bottom, no duplicate checkbox) */}
              <p className="text-[10px] text-center text-[#687087] pt-2 leading-relaxed font-normal">
                ...
                <a href="/terms" className="text-[#5125D8] font-semibold hover:underline">Privacy Policy</a> and{' '}
                <a href="/privacy" className="text-[#5125D8] font-semibold hover:underline">Terms of Service</a>.
              </p>
            </form>
          </div>
        </div>
